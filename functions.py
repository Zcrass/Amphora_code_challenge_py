...
def get_vcf_names(vcf_path):
    ''' 
    This function identify the start of the vcf table and return the names of it.
...
def import_geno(file, format):
    ''' 
    This function reads a CSV or VCF file from one individual sample and import the content as a data frame.
    
    VCF files should include ten columns: ['CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL','FILTER', 'INFO', 'FORMAT', 'individual genotye'].
    VCF files could be plain text or gziped.
    CSV files should include five columns: ['CHROM;POS', 'REF', 'ALT', 'ALT.1', 'individual genotye'].
    ...
    Returns:
        A dataframe of 5 columns: ['CHROM', 'POS', 'REF', 'ALT', individual genotype].
...
def geno2num(genotype):
    '''
    This function 'turn a series of genotype format values (*|*) into a series of numeric genotypes (0, 1, 2)
    ...
    Parameters:
        genotype: A series of genotype values in format '*|*'.
  
    Returns:
        A series of int values representing the genotypes.

    '''
    genotype = genotype.str.replace('1|1', '2', regex=False)
    genotype = genotype.str.replace('1|0', '1', regex=False)
    genotype = genotype.str.replace('0|1', '1', regex=False)
    genotype = genotype.str.replace('0|0', '0', regex=False)
    return genotype
